###################################################################################################
#          RAKE tasks for the deployment to S3               
#               - sync assets to staging  --> s3:push:to_staging (copy files from public/assets to s3
#                                       to cwcdn0/assets/GitRandHash/ )
#
#
#         USES:
#              - upload to S3            --> s3:uploadToS3 
####################################################################################################
...
namespace :s3 do
  ...
  namespace :push do
    task :to_staging => [:environment] do
      Rake::Task["s3:uploadToS3"].execute({ to: 'cwcdn0' })
    end
  end        
